    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (int sizex in sizes)
            {
                ...
                var context = new Context(new SortClient(new CountingSort()));
                context.ExecuteStrategy(array);

                var contextClone = (Context)context.Clone();
                contextClone.SetSortAbstraction(new SortClient(new InsertionSort()));
                contextClone.ExecuteStrategy(array);

                var contextClone2 = (Context)context.Clone();
                contextClone2.SetSortAbstraction(new SortClient(new QuickSort()));
                contextClone2.ExecuteStrategy(array);

                var contextClone3 = (Context)context.Clone();
                contextClone3.SetSortAbstraction(new SortClient(new MargeSort()));
                contextClone3.ExecuteStrategy(array);

                var contextClone4 = (Context)context.Clone();
                contextClone4.SetSortAbstraction(new SortClient(new ShellSort()));
                contextClone4.ExecuteStrategy(array);

                // Adicione aqui outras estratégias de ordenação, se necessário
            ...
            // Repetição para diferentes tamanhos e estratégias
            foreach (int sizex in sizes)
            {
            ...
            foreach (int sizex in sizes)
            {
                ...
                int[] array = generateArray.CreateArray(sizex);
                stopWatch1.Start();
                var context = new Context(new SortClient(new InsertionSort()));
                context.ExecuteStrategy(array);
                stopWatch1.Stop();
            ...
            foreach (int sizex in sizes)
            {
                ...
                int[] array = generateArray.CreateArray(sizex);
                stopWatch2.Start();
                var context = new Context(new SortClient(new QuickSort()));
                context.ExecuteStrategy(array);
                stopWatch2.Stop();
            ...
            foreach (int sizex in sizes)
            {
                ...
                int[] array = generateArray.CreateArray(sizex);
                stopWatch3.Start();
                var context = new Context(new SortClient(new MargeSort()));
                context.ExecuteStrategy(array);
                stopWatch3.Stop();
            ...
            foreach (int sizex in sizes)
            {
                ...
                int[] array = generateArray.CreateArray(sizex);
                stopWatch4.Start();
                var context = new Context(new SortClient(new ShellSort()));
                context.ExecuteStrategy(array);
                stopWatch4.Stop();
